...
<h2 className='capitalize text-lg sm:text-xl md:text-2xl flex-1 md:text-center'>{exercise.name.replaceAll("_" , " ")}</h2>
<p className='text-sm text-slate-400 capitalize'>{exercise.type}</p>
        ...
        <div className='flex flex-col'>
          ...
          <p className='capitalize'>{exercise.muscles.join(' & ')}</p>
                  ...
                  <div className='flex flex-col bg-slate-950 rounded gap-2'>{exercise.description.split('___').map((val) => {
                    return (
...
<p className="font-medium">{exercise[info]}</p>
                  ...
                  <button onClick={handleSetIncrement} className='flex flex-col p-2 rounded border-[1.5px] duration-200 border-solid cursor-pointer border-blue-900 w-full hover:border-blue-400'>
